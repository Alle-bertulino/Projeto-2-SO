...
int fifo(int8_t** page_table, int num_pages, int prev_page,
    ...
    //Faz a verificação do endereço de memoria, o comparando com a moldura de pagina, caso seja verdadeiro retorna a pagina.
...
int second_chance(int8_t** page_table, int num_pages, int prev_page,
                  int fifo_frm, int num_frames, int clock) {
    //Laços de repetição para percorrer a tabela             	
    for (int i = 0; i < num_pages; i++){
    	for(int j = 0; j < num_pages; j++){
            // Faz a verificação se o endereço de memoria de i e j são iguais 
    	    if (page_table[j][PT_FRAMEID] == i){
    	    	// Verifica se o bit R é igual a 0, se for, a pagina a ser removida tem que ser retornada
    		    if (page_table[j][PT_REFERENCE_BIT] == 0){
    		    	return j;
                }else{
                	//Se não, recebe uma segunda chance e atualiza o valor do bit R para 0
               	    page_table[j][PT_REFERENCE_BIT] = 0;
               	   	//Com isso recebe a atualização da ultima pagina acessada (prev_page)
    		        prev_page = j;
    		        break;
	         	}   		
            }
...
int nru(int8_t** page_table, int num_pages, int prev_page,
    ...
    for(int i = 0; i < num_pages; i++){
        //Faz a verificação para saber se o endereço está presenta na tabela
        if(page_table[i][PT_MAPPED] !=0){
        	// Verifica se o bit M e bit R são iguais a 0, se for, a pagina a ser removida foi encontrada, ocorre o retorno
    ...
    //Faz o mesmo processo da primeira função, porém verificando as demais classes proxsiveis 
    for(int i = 0; i < num_pages; i++){
        if(page_table[i][PT_MAPPED] !=0){
...
int aging(int8_t** page_table, int num_pages, int prev_page,
    ...
    int var = 0, contador = 0, prox = 0;
    while(contador != num_pages){
    	//Faz a verificação para saber se o endereço está presenta na tabela, caso não esteja incrementa o contador para a proxima interação 
        if(page_table[contador][PT_MAPPED] == 0){
        	contador++;
		}else{
			// A variavel prox pega a primeira proxição de um endereço valido, com base nisso a variavel var recebe o argumento do contador sempre que encontrar um valor var ao anteriormente informado 
            prox ++;
            if(prox == 1) {
                var = contador;
            }
            if(page_table[var][PT_AGING_COUNTER] > page_table[contador+1][PT_AGING_COUNTER]){
                var=contador;
            }
            contador++;
        }
    }
    // Ao final retorna o valor encontrado
    return var;
}
